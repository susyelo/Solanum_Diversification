...
# 			}
		...
		states_indices = all_states_list[states_allowed_TF]
				...
				if (is.numeric(treepiece) )
					...
					if (do_exponentiation == TRUE)
						...
						if (sparse==FALSE)
							...
							actual_probs_after_forward_exponentiation = calc_prob_forward_onebranch_sparse(relprobs_branch_bottom=ancprobs_at_subbranch_bottom[states_allowed_TF], branch_length=subbranch_length, tmpQmat_in_REXPOKIT_coo_fmt, coo_n=coo_n, anorm=anorm, check_for_0_rows=TRUE)
					...
					if (i == num_timeperiods) # You are at the bottom tree piece, just use root node
						...
						if ((is.numeric(phy2$root.edge) == TRUE) && (!is.null(phy2$root.edge)) && (phy2$root.edge > 0))
							...
							if (sparse==FALSE)
								...
								# Sparse matrix exponentiation
								actual_probs_after_forward_exponentiation = calc_prob_forward_onebranch_sparse(relprobs_branch_bottom=ancprobs_at_subbranch_bottom[states_allowed_TF], branch_length=root_edge_length, tmpQmat_in_REXPOKIT_coo_fmt, coo_n=coo_n, anorm=anorm, check_for_0_rows=TRUE)
								}
