...
#' 
tipranges_to_tip_condlikes_of_data_on_each_state <- function(tipranges, phy, states_list=NULL, maxareas=length(getareas_from_tipranges_object(tipranges)), include_null_range=TRUE, useAmbiguities=FALSE, trait_as_tip_condlikes=NULL)
	{
	default='
	tipranges
	phy
	...
	if (is.null(states_list))
		...
		states_list = areas_list_to_states_list_new(areas=areanames, include_null_range=include_null_range, maxareas=maxareas)
		}
	...
	if ((useAmbiguities == FALSE) || (sum(tipranges@df == "?") == 0))
		...
		# Here, the only meaningful thing is 0s; exclude these states/ranges
		tipranges_orig
		tipranges_flipped = tipranges_orig
		
		# Get the letter code ranges for each binary state
		tipranges_flipped@df[tipranges_flipped@df == "?"] = "z"
		tipranges_flipped@df[tipranges_flipped@df == "1"] = "z"
		tipranges_flipped@df[tipranges_flipped@df == "0"] = "1"
		tipranges_flipped@df[tipranges_flipped@df == "z"] = "0"
		letter_code_ranges = binary_ranges_to_letter_codes(tipranges_flipped, areanames)
		names(letter_code_ranges) = NULL
		letter_code_ranges
		# These are the ranges that are DISALLOWED
		...
		# Make sure the letter code ranges are in the same order as the 
		# phylogeny tips
		tipranges_df_order = match(phy$tip.label, rownames(tipranges_flipped@df))
		letter_code_ranges = letter_code_ranges[tipranges_df_order]
		...
		# But, if you have any rows that are actually null range (e.g. fossils), set those to likelihood 1!
		rows_with_all0s_TF = check_tipranges_for_allQs(tipranges_orig, symbol_to_check="0")
...
#' 
getranges_from_LagrangePHYLIP <- function(lgdata_fn="lagrange_area_data_file.data", block_allQs=TRUE)
	{
