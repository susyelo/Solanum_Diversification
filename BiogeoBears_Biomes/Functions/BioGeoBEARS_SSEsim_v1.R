...
SSEsim_to_files <- function(SSEsim_results, simdir, fossils_older_than=0.001, printlevel=2, write_files=TRUE)
	...
	cladogenetic_event_labels = label_table_of_cladogenetic_events(table_of_cladogenetic_events_translated, states_list, ranges_list, areanames=areanames)
	...
	if (naTF == FALSE)
		{
		anagenetic_event_labels = label_table_of_anagenetic_events(table_of_range_change_events_translated, states_list, ranges_list, areanames=areanames)
...
label_table_of_anagenetic_events <- function(table_of_range_change_events_translated, states_list, ranges_list, areanames, track_dispersal_dest=TRUE)
	{
	# Check for all NAs
	if (all(is.na(SSEsim_results$table_of_range_change_events_translated)))
		{
	...
	if (track_dispersal_dest == TRUE)
		{
		anagenetic_event_labels = as.data.frame(cbind(event_type, event_txt, dispersal_to, extirpation_from))
...
label_table_of_cladogenetic_events <- function(table_of_cladogenetic_events_translated, states_list, ranges_list, areanames=areanames, track_dispersal_dest=TRUE)
	{
	...
	if (track_dispersal_dest == TRUE)
		{
		cladogenetic_event_labels = as.data.frame(cbind(event_type, event_txt, dispersal_to))
...
SSEsim_results_processed_to_anagenetic_events_table <- function(SSEsim_results_processed, simtr_complete_or_observed="complete", tmptr_table=NULL, convert_clado_events_w1desc=FALSE)
	...
	# Label event types as just "d", "e", or "j"
