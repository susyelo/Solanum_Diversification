...
#' is tree height
#' @skiplabels If TRUE, skip the nodelabels command, resulting in plotting just the 
#' tree. (Yes, you could have just used \code{FALSE}).  Default \code{FALSE}.
#' @plot_stratum_lines If TRUE (default), plot dotted lines at the stratum 
#' boundaries, *if* it's a time-stratified results object.
#' @simplify_piecharts If TRUE, just plot one slice for the maximum probability, 
...
#' times 1000+nodes). Default FALSE.
#' @tipboxes_TF Plot the tip boxes (tip states)?  Default TRUE.
...
get_statesColors_table <- function(areanames=c("K","O","M","H"), plot_null_range=FALSE)
	...
	colors_list_for_states = mix_colors_for_states(colors_matrix, states_list_0based_index, include_null_range=plot_null_range)
	colors_list_for_states
...
plot_BSM <- function(results_object, clado_events_table, stratified, analysis_titletxt="Stochastic map", addl_params=list(), plotwhat="text", label.offset=NULL, tipcex=0.8, statecex=0.7, splitcex=0.6, titlecex=0.8, plotsplits=TRUE, plotlegend=FALSE, legend_ncol=NULL, legend_cex=1, cornercoords_loc="manual", tr=NULL, tipranges=NULL, if_ties="takefirst", pie_tip_statecex=0.7, juststats=FALSE, xlab="Millions of years ago", manual_ranges_txt=NULL, root.edge=TRUE, colors_list_for_states, skiptree=FALSE, show.tip.label=TRUE, tipcol="black", dej_params_row=NULL, plot_max_age=NULL, skiplabels=FALSE, plot_stratum_lines=TRUE, include_null_range=NULL, plot_null_range=FALSE)
	{
	scriptdir = np(system.file("extdata/a_scripts", package="BioGeoBEARS"))
