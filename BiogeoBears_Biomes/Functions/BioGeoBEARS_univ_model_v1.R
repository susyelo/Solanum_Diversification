	...
	if (is.null(BioGeoBEARS_run_object$custom_Qmat_fn_text) == TRUE)
		...
		if (traitTF == TRUE)
			...
			for (traitstate_i in 1:ntrait_states)
				{
				# For a particular trait state, apply the dispersal 
				# multipliers
				trait_multiplier = BGB_trait_model_params_table$est[traitstate_i]
				#print("trait_multiplier:")
				#print(trait_multiplier)
			
				dmat_times_d = trait_multiplier * dispersal_multipliers_matrix * matrix(d, nrow=length(areas), ncol=length(areas))
				amat = trait_multiplier * dispersal_multipliers_matrix * matrix(a, nrow=length(areas), ncol=length(areas))
...
#		print(m)
		...
		#print(jts_matrix)
		model_results = calc_loglike_sp(tip_condlikes_of_data_on_each_state=tip_condlikes_of_data_on_each_state, phy=phy, Qmat=Qmat, spPmat=NULL, return_what="all", sparse=force_sparse, use_cpp=TRUE, input_is_COO=force_sparse, spPmat_inputs=spPmat_inputs, printlevel=0, cluster_already_open=cluster_already_open, calc_ancprobs=TRUE, fixnode=fixnode, fixlikes=fixlikes, include_null_range=BioGeoBEARS_run_object$include_null_range, m=m, jts_matrix=jts_matrix, BioGeoBEARS_model_object=BioGeoBEARS_run_object$BioGeoBEARS_model_object)
