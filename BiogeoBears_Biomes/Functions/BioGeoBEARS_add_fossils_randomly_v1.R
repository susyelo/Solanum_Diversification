...
add_fossils_to_many_trees <- function(nexfn, burnin, numtrees, xls, living_tipranges_fn, fn_prefix="tr", fn_suffix="", replace=FALSE)
	...
	for (i in 1:length(treenums_to_use))
		...
		# Are there any taxa in the tree that are not in the geography file?  Cut those.
		geogtips_missing_from_tree_TF = (tr$tip.label %in% row.names(tipranges@df)) == FALSE
		num_missing = sum(geogtips_missing_from_tree_TF)
		if (num_missing > 0)
			{
			txt = paste0("Wait. add_fossils_to_many_trees() has noticed that your geography file '", living_tipranges_fn, "' has ", num_missing, " names that are not in the treefile, '", nexfn, "', tree #", treenum_to_use, ". Running drop.tip() to remove them from the tree. Really, you should get this kind of basic thing straightened out before running a complex analysis like this.\n")
			cat(txt)
			warning(txt)
			tips_to_drop = tr$tip.label[geogtips_missing_from_tree_TF]
		...
		treetips_missing_from_geog_TF = (row.names(tipranges@df) %in% tr$tip.label) == FALSE
		num_missing = sum(treetips_missing_from_geog_TF)
		if (num_missing > 0)
			{
			txt = paste0("Wait. add_fossils_to_many_trees() has noticed that your tree file '", nexfn, "', tree #", treenum_to_use, " has ", num_missing, " tipnames that are not in the geography file, '", living_tipranges_fn, "'. Dropping them from the geography file. Really, you should get this kind of basic thing straightened out before running a complex analysis like this.\n")
			cat(txt)
			warning(txt)
			tips_to_drop = row.names(tipranges@df)[geogtips_missing_from_tree_TF]
			tmpdf = tipranges@df
			tmpdf2 = tmpdf[treetips_missing_from_geog_TF==FALSE, ]
			tipranges@df = tmpdf2
		...
		tr_w_fossils_and_fossils_added_list = add_fossils_from_xls_randomly(tr=tr, xls=xls)
